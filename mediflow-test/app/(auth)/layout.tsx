  ...
  return (
    <div className="min-h-screen w-full bg-[#071B34] flex flex-col font-sans relative">
      ...
      <header className="w-full bg-[#071B34] border-b border-white/10 px-4 sm:px-8 py-3 flex items-center justify-between z-30 shrink-0 shadow-md">
        ...
        {/* Right: Install App | Theme | Sign In/Sign Up */}
        <div className="flex items-center space-x-2 sm:space-x-3">
          {/* Install App Button (Works Before Login) */}
          ...
          <button
            ...
            className="px-2.5 py-1.5 rounded-xl bg-white/10 hover:bg-white/15 text-white border border-white/20 transition-all text-xs font-bold flex items-center space-x-1.5 cursor-pointer"
          >
            <span>{isDark ? "🌙" : "☀️"}</span>
          ...
          {isLoginPage ? (
            <Link
              href="/signup"
              className="px-3 py-1.5 sm:px-3.5 sm:py-2 rounded-xl bg-blue-600 hover:bg-blue-500 text-white font-bold text-xs shadow-sm transition-all cursor-pointer whitespace-nowrap"
            >
              Sign Up
            </Link>
          ) : isSignupPage ? (
